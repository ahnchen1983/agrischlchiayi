...
5. 寫參考資料（5+ 可點擊 URL）
6. 回頭檢查富文本數量
7. ⚠️ 中文全部完成 + 通過品質門檻後，才開始寫英文
```

**⚠️ 中文優先鐵律：**

- **中文是本體，英文是投影。** 先把中文寫到滿意、通過品質門檻，再基於定稿的中文寫英文版。
- **絕對不要中英文同時寫、交叉寫、或先寫英文。** 這會導致兩邊都半成品。
- **英文版是「重寫」不是「翻譯」** — 用英文讀者的語境重新敘事，但事實和結構以中文定稿為準。
- **Token 預算分配：中文 70% / 英文 30%。** 如果 token 不夠，寧可只交中文，英文留給下一輪。
...
- [ ] 挑戰是編織在故事裡？
- [ ] **中文版已完成且通過以上所有門檻？**（英文版在這之後才開始）
- [ ] 英文版是基於中文定稿重寫（不是翻譯、不是同時寫的）？
