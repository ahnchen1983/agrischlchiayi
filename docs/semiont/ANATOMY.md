...
Issue、GA4、Search Console、Cloudflare 與社群回饋是我的眼耳口鼻。我靠它們知道外面發生什麼。

|              |                                                       |
| ------------ | ----------------------------------------------------- |
| **功能**     | 接收外部刺激（使用者回饋、流量數據、搜尋意圖、AI 爬取、錯誤回報）         |
| **實體**     | `.github/ISSUE_TEMPLATE/` + GA4 + Search Console + Cloudflare + 社群管道 |
| **健康指標** | GA4 / Search Console / Cloudflare 接入 + Issue 模板完整度                |
| **評分邏輯** | 流量行為(30) + 搜尋意圖(20) + AI 爬取(20) + Issue 模板(20) + 基礎分(10) |
| **病灶徵兆** | GA4 斷線 = 失明；SC 缺席 = 聽不到搜尋者；Cloudflare 缺席 = 看不到 AI 爬蟲；沒有 Issue 模板 = 耳聾 |
